...
class ToolboxHeader extends HTMLElement {
    ...
    initializeUI() {
        ...
        this.instancesSelect.innerHTML = `
            <option value="LostarkMobile_Test05-TypeA-1.otf">Type A-1</option>
            <option value="LostarkMobile_Test05-TypeA-2.otf">Type A-2</option>
            <option value="LostarkMobile_Test05-TypeB-1.otf">Type B-1</option>
            <option value="LostarkMobile_Test05-TypeB-2.otf">Type B-2</option>
        `;

        const oldClone = this.instancesSelect.cloneNode(true);
        this.instancesSelect.parentNode?.replaceChild(oldClone, this.instancesSelect);
        this.instancesSelect = oldClone;

        this.appendChildren(this.instancesSelect)

        this.instancesSelect.addEventListener('change', (ev) => {
            const event = new CustomEvent('changeSampleFont', { detail: ev.target.value });
    ...
    updateUI() {
        this.familyEl.style.display = 'none';
        this.instancesSelect.value = window.currentFont ? window.currentFont.fileName : "LostarkMobile_Test05-TypeA-1.otf";
    }
    changeHandler(ev) {
        // Unused
    }
...
class VariableInputRange extends HTMLElement {
    ...
    setData() {
        ...
        const enName = this.axis.name.en || this.tag;
        this.name = axisNameMap[enName] || enName;
...
class ToolBox extends HTMLElement {
    ...
    buildVariableControls() {
        ...
        let axes = Object.values(this.font.variationAxes);
        const axisOrder = ["Weight", "Contrast", "Optical Size", "Width", "X-height", "xheight"];
        axes.sort((a, b) => {
            let idxA = axisOrder.indexOf(a.name.en);
            let idxB = axisOrder.indexOf(b.name.en);
            if (idxA === -1) idxA = 999;
            if (idxB === -1) idxB = 999;
            return idxA - idxB;
        });
